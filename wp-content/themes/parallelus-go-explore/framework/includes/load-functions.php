...
if ( !function_exists( 'get_options_data' ) ) {
	...
	function get_options_data( $key, $option = false, $default = null ) {
		...
		$original_key = $key;
		$wpml_key = get_wpml_key();
		$key = $shortname . $key . $wpml_key;
		...
		if (isset($option_value)) {
			...
			//if(has_filter( 'wpml_translate_options_object', 'wpml_translate_options_object_func' ))
			//	$value = apply_filters('wpml_translate_options_object', $value);
		} else {
...
if(!function_exists('get_wpml_key')) {
	function get_wpml_key() {
		$wpml_key = '';
		if(defined('ICL_LANGUAGE_CODE')) {
			global $sitepress;
			$default_language = $sitepress->get_default_language();
			$wpml_key = ($default_language == ICL_LANGUAGE_CODE)? '' : '_'.ICL_LANGUAGE_CODE;
		}
		return $wpml_key;
	}
}
