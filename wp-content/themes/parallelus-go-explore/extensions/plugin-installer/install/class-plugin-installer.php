 ...
 * 
 * -------------------------------------------------------------------------------------
 * IMPORTANT - THEMEFOREST REVIEWERS. READ THIS.
 * -------------------------------------------------------------------------------------
 * 
 * 
 * 
 * 
 * THIS FILE INCLUDES THE SERURITY FIXES REFERENCED HERE: 
 * http://themeforest.net/forums/thread/security-vulnerability-affecting-wordpress-plugins-and-themes/173011
 * 
 * AGAIN, FOR THE THEMEFOREST REVIEWERS... THIS FILE DOES INCLUDE THE FIXES REFERENCED HERE: 
 * http://themeforest.net/forums/thread/security-vulnerability-affecting-wordpress-plugins-and-themes/173011
 * 
 * 
 * 
 ...
 *
 * @package   Heaily Modified - TGM-Plugin-Activation with fixes for the security flaw (http://themeforest.net/forums/thread/security-vulnerability-affecting-wordpress-plugins-and-themes/173011)
 * @version   v2.4.2 (based on TGM Plugin Activation but heavily modified)
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					if ( isset($action_links) && !empty($action_links) ) {
						...
						// Activate link?
						if ( isset($activate_link) && $activate_link && isset($action_links['activate']) ){
							$activateLink = $action_links['activate'] . ' &nbsp; ';
						} else {
							$activateLink = '';
						}
						// Install link?
						if ( isset($install_link) && $install_link && isset($action_links['install']) ){
